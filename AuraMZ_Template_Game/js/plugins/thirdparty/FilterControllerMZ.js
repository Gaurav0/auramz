 ...
 * @value ExcludeSpecificPicture
 * @option ExcludeSpecificEnemy
 * @value ExcludeSpecificEnemy
 * 
 * @arg targetIds
...
(function() {
	...
	Filter_Controller.targetType["ExcludeSpecificEnemy"] = "ExcludeSpecificEnemy";
	...
	// defenition of each TargetType's target
	{
		...
		targetGetter[Type.ExcludeSpecificEnemy] = function(targetIds) {
			if (!this._spriteset) return [];
			if (!this._spriteset._pictureContainer) return [];

			const picContainer = this._spriteset._enemySprites;
			const targets = picContainer.clone();
			targetIds.forEach(targetId => {
				if (picContainer[targetId-1]) {
					if (targetId >= 1) {
						targets.splice(targetId - 1, 1);
					}
				}
			});
			
			targets.push(...this._spriteset._actorSprites);
			return targets;
		};
	}
