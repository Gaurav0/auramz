...
(() => {
	...
	Game_Actor.prototype.passiveSkills = function() {
		let _passiveSkills = [];
		const addedSkills = this.baseTraitObjects()
		...
		for (const id of this._skills.concat(addedSkills)) {
	...
	Game_Actor.prototype.paramPlus = function(paramId) {
		...
		for (const passive of this.passiveSkills()) {
			if (passive) {
				const add = passive.meta[ADD_NOTE_TAG] ? eval(passive.meta[ADD_NOTE_TAG]) : 0;
				const multiplier = passive.meta[MULTIPLIER_NOTE_TAG] ? eval(passive.meta[MULTIPLIER_NOTE_TAG]) : 1;
				value += Math.ceil((passive.params[paramId] + add) * multiplier);
			}
		}
		return value;
	};

	Game_Actor.prototype.baseTraitObjects = Game_Actor.prototype.traitObjects;
	Game_Actor.prototype.traitObjects = function() {
		let objects = this.baseTraitObjects();
